...
namespace Blaze{
	...
	public class Scanner:PointReceiver{
		...
		/// <summary>Computed when the first vector is rasterised. BlurSpread / 2f.</summary>
		private float HalfBlurSpreadF;
		/// <summary>Computed when the first vector is rasterised. BlurSpread / 2.</summary>
		...
		/// <summary>The amount to extrude an SDF glyph by.</summary>
		private float SDFExtrudeBy;
		
		
		/// <summary>The amount to extrude an SDF glyph by.</summary>
		public float ExtrudeBy{
			get{
				return SDFExtrudeBy;
			}
			set{
		...
		public void Start(){
			...
			// And half it:
			HalfBlurSpreadF=(float)BlurSpread/2f;
			
			HalfBlurSpread=BlurSpread/2;
			
			SDFExtrudeBy=-HalfBlurSpreadF/ScalarY;
